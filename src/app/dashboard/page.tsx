...
import { Plus, QrCode, ArrowRight, Heart, Lock, Globe, Zap, Eye, CheckCircle2 } from 'lucide-react';
...
export default async function DashboardOverview() {
  ...
  // Check which memorials have an assigned medallion
  // Look in medallion_codes for memorial_id OR assigned_page_id matches
  const memorialIds = (memorials || []).map(m => m.id);
  const medallionMap: Record<string, string> = {}; // memorial_id → code
              ...
              return (
                <div key={memorial.id} className="bg-white rounded-3xl p-6 shadow-sm border border-stone-100 flex flex-col group hover:shadow-md transition duration-300 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    ...
                    {/* Dynamic medallion badge */}
                    {assignedCode ? (
                      <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
                        <CheckCircle2 className="w-3 h-3" /> {assignedCode}
                      </div>
                    ) : (
                      <div className="text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-slate-50 text-slate-400 border border-slate-100 flex items-center gap-1.5">
                  ...
                  {!memorial.is_live && (
                    <div className="mb-4 bg-gradient-to-br from-amber-50 to-stone-50 border border-amber-100 rounded-2xl p-4">
                      <p className="text-xs text-slate-600 font-medium mb-3 flex items-center gap-1.5">
                        <Zap className="w-3.5 h-3.5 text-amber-500" /> Seite freischalten für CHF 49.–
                      </p>
                  ...
                  <div className="mt-auto border-t border-stone-100 pt-4 flex gap-2">
                    ...
                    <Link href={`/dashboard/edit/${memorial.id}`}
                      className="flex-1 text-center py-2.5 rounded-xl border border-stone-200 bg-stone-50 text-slate-600 text-sm font-medium hover:bg-white transition shadow-sm">
                      {memorial.is_live ? 'Verwalten' : 'Bearbeiten'}
                    </Link>
                    <Link href={assignedCode ? `/dashboard/medaillon/${memorial.id}` : '/medaillons'}
                      ...
                      {assignedCode ? 'QR-Code' : 'Medaillon'} <ArrowRight className="w-3.5 h-3.5 opacity-70" />
                    </Link>
                  </div>
