...
import { Users, Mail, MapPin, ShoppingBag, Calendar, ShieldCheck } from 'lucide-react';
...
export default async function AdminCustomersPage() {
  ...
  // Load ALL profiles — show admins with badge, don't exclude them
  ...
  const customers = allProfiles.filter(p => !p.is_admin);
  ...
  return (
    <div className="p-8 md:p-12 max-w-7xl">
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm">{customers.length} Kunden · {allProfiles.length} Nutzer gesamt</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {[
          ...
          { label: 'Nutzer gesamt', value: allProfiles.length, icon: Calendar, color: 'text-amber-700 bg-amber-50' },
        ].map(s => (
          ...
          <table className="w-full text-left text-sm min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {allProfiles.map(profile => {
                const pages = pageCounts[profile.id] || { total: 0, live: 0 };
                const orders = orderCounts[profile.id] || 0;
                const fullName = [profile.first_name, profile.last_name].filter(Boolean).join(' ');
                const location = [profile.city, profile.country].filter(Boolean).join(', ');
                return (
                  <tr key={profile.id} className="hover:bg-stone-50 transition">
                    ...
                    <td className="px-4 py-4 text-xs text-slate-400">
                      {profile.created_at ? new Date(profile.created_at).toLocaleDateString('de-CH', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '—'}
                    </td>
                  </tr>
                );
              })}
